 ...
 * 承認済みタブ
 */
const ApprovedList = (props: UnappliedListProps) => {
  ...
  const isSelected = (userId: string) => selectedData.indexOf(userId) !== -1;

  const handleClick = (userId: string) => {
    let newSelected: string[] = JSON.parse(JSON.stringify(selectedData));
  ...
  const createExcel = (e: any, userId: string) => {
    e.preventDefault();
    // Workbookの作成
    ...
    let blueCells: string[] = []; // 青セル
    let redCells: string[] = []; //赤セル
    let greenCells: string[] = ["B4", "J4", "R4"]; //緑セル
  ...
  const outputExcel = async (e: any, userId: string) => {
  ...
  const convertToPDF = async (workbook: any, name: string) => {
    // UInt8Arrayを生成
    const uint8Array = await workbook.xlsx.writeBuffer();
    // ExcelファイルをBlobに変換
    const blob = new Blob([uint8Array], { type: "application/octet-binary" });
  ...
  const handleConvertClick = async (workbook: any, name: string) => {
  ...
  const bulkOutputExcel = async (e: any) => {
    const JSZip = require("jszip");
    const zip = new JSZip();
    const excelFiles: excelFileType[] = [];
